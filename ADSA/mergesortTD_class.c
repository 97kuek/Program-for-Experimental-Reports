...
/*
  目的：
    昇順に整列された左半分 a[l..m] と、昇順の右半分 a[m+1..r] を
    1 本の昇順配列 a[l..r] に「マージ」する。

  ポイント（ビトニック・マージ）：
    1) 補助配列 aux に対し、
       - 左半分はそのまま（昇順のまま） aux[l..m] にコピー
       - 右半分は「逆順で」 aux[m+1..r] にコピー（←ここが重要）
         → aux[l..r] 全体は「左が小→大、右が大→小」という
           “ビトニック（片側昇順＋片側降順）”な形になる
    2) あとは i=l（左端）と j=r（右端）から、常に小さい方を取り出して a[k] に格納。
       - 右側を逆順にしてあるため、min 取り出し時に番兵や尽きた判定が不要。
*/
...
#define less(A, B)    (key(A) <  key(B))                                        // ← B も key() を通す
#define exch(A, B)    do { Item t_ = (A); (A) = (B); (B) = t_; } while (0)
